...
//  Varela's code available at https://github.com/aslr/Seascape
//
//  Modifications to remove unnecessary mouse code by zzywysm, 2019
...
kernel void compute_function(texture2d<float, access::write> output [[texture(0)]],
                             ...
                             device const float &time [[buffer(0)]])
{
    ...
    float timee = iTime * .3 + .01;
    
    // ray
    float3 ang = float3(sin(timee*3.)*.1,sin(timee)*.2+.3,timee);
    float3 ori = float3(0.,3.5,timee*5.);
